...
def grille(cases):
    ...
    for i in range(nb_cases):
        for j in range(nb_cases):
            ...
            CG.remove(colors)
            grillage.create_rectangle((pos1x, pos1y), (pos2x, pos2y), fill=colors)
...
def choix_parametres():
    ...
    global k, n, p, T
    fenetre1 = tk.Tk()
    tk.Label(fenetre1, text="Paramètre p (la probabilité d'avoir une case eau)").grid()
    Param1 = tk.Entry(fenetre1)
    Param1.insert(0, p)
    Param1.grid()
    tk.Label(fenetre1, text="Paramètre n (le nombre de fois où on répète l'automate)").grid()
    Param2 = tk.Entry(fenetre1)
    Param2.insert(0, n)
    Param2.grid()
    tk.Label(fenetre1, text="Paramètre T (la valeur du voisinage)").grid()
    Param3 = tk.Entry(fenetre1)
    Param3.insert(0, T)
    Param3.grid()
    tk.Label(fenetre1, text="Paramètre k (l'ordre du voisinage de Moore)").grid()
    Param4 = tk.Entry(fenetre1)
    Param4.insert(0, k)
    Param4.grid()
    tk.Button(fenetre1, text="valider", command=fenetre1.quit).grid()
    fenetre1.mainloop()
    p = float(Param1.get())
    n = int(Param2.get())
    T = int(Param3.get())
...
def choix_taille():
    ...
    fenetre1 = tk.Tk()
    tk.Label(fenetre1, text="Saisir le nombre de colonnes de la grille").grid()
